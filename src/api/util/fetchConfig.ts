...
import { FEE_TIER, Q192, sqrt, tokenCompare } from "../../constants/global";
...
export const getCurPrice = async (
  poolAddress: string,
  tokenIn: IConfigToken,
  tokenOut: IConfigToken
): Promise<string> => {
  const Tezos = await dappClient().tezos();
  const pool = await Tezos.wallet.at(poolAddress);
  const storage: any = await pool.storage();

  const sqrtPriceX96: BigNumber = await storage["slot0"].sqrtPriceX96;
  const priceX192 = sqrtPriceX96.exponentiatedBy(2)

  const cond = tokenCompare(tokenIn, tokenOut);
  if (cond === 0 || cond === 2) return "false";

  let price;
  if (cond === -1) {
    price = new BigNumber(Q192.toString()).div(priceX192).multipliedBy(10 ** (tokenIn.decimals - tokenOut.decimals))
  } else {
    price = new BigNumber(priceX192).div(new BigNumber(Q192.toString())).multipliedBy(10 ** (tokenOut.decimals - tokenIn.decimals))
  }

  return price.toString() ?? "false";
};
